	...
	*/
	class neuralnet_t
	{
		public:
			...
			*/
			neuralnet_t(const std::vector<size_t>& layers,const std::vector<double>& weights,double king);
			...
			*/
			double evaluate(const std::vector<int>& inputs,
			...
			*/
			double evaluate(const std::vector<int>& inputs,const size_t sigmoid_index,
		...
		private:
			...
			std::vector<node_t> piece_count_layer_m;
			double king_m;
	};
}
